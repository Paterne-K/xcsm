...
class DHomeController extends Controller
{
    ...
    public function savePdf(Request $request){
        ...
        if($file != null && ($file->getSize() / (1024 * 1024)) <= 175){
            ...
            console.log("Biyong   "+$path_image);
        }

        /*$target_dir = "xmoddledata/";

        $target_name_file = $id."_".$name;

        mkdir($target_dir.$target_name_file);

        $target_dir_file = $target_dir.$target_name_file."/";

        $target_dir_image = $target_dir_file."images";

        mkdir($target_dir_image);

        $target_file = $target_dir.$target_name_file."/".$file;

        move_uploaded_file($_FILES["file"]["tmp_name"], $target_file);

        $file_xml = $target_dir_file.$name.".xml";

        exec("java -jar xmoddledata/PdfBox.jar ".$target_file." ".$target_dir_image." ".$name." ".$file_xml);*/
