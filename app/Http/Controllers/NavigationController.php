    ...
    public function lectureNavigation(Request $request) {
        {
            ...
            for ($i = 0; $i < $description->attributes()->nbrParties; $i++) {
                ...
                if($description->partie[$i]->chapitre[$j] == null){
                    $titre["fils"] = $nav_chapitres;
                    $nav_parties["".$i] = (object) $titre;
                }
                else if(!(isset($description->partie[$i]->chapitre[$j]->attributes()->type) && $description->partie[$i]->chapitre[$j]->attributes()->type == "mal classe")) {
                    $titre["fils"] = $nav_chapitres;
                    $nav_parties["".$i] = (object) $titre;
                }
